...
from convert import write_to_bin
...
# Important settings
tf.app.flags.DEFINE_string('mode', 'train', 'must be one of train/eval/decode/inference')
...
#  FLAGS.data_path=write_to_bin(FLAGS.data_path)
#  print(FLAGS.data_path)
  #FLAGS.data_path=FLAGS.data_path+'.txt'
...
def run_eval(model, batcher, vocab):
  ...
  while True:
    ...
    tf.logging.info('These was for step: {}'.format(train_step))
...
def main(unused_argv):
  ...
  if FLAGS.mode == 'inference':
    FLAGS.data_path=write_to_bin(FLAGS.data_path)
    tf.logging.info(FLAGS.data_path)
  # If in decode mode, set batch_size = beam_size
  # Reason: in decode mode, we decode one example at a time.
  # On each step, we have beam_size-many hypotheses in the beam, so we need to make a batch of these hypotheses.
  if FLAGS.mode == 'decode' or FLAGS.mode == 'inference':
    FLAGS.batch_size = FLAGS.beam_size

  # If single_pass=True, check we're in decode mode
  if FLAGS.single_pass and (FLAGS.mode!='decode' or FLAGS.mode == 'inference'):
  ...
  if hps.mode == 'train':
    ...
    raise ValueError("The 'mode' flag must be one of train/eval/decode/inference")
